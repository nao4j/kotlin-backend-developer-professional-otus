...
fun validationDescriptionCorrect(command: Command, processor: QuestionProcessor) = runTest {
    ...
    assertEquals("abc", ctx.questionValidated.description)
}

@OptIn(ExperimentalCoroutinesApi::class)
fun validationDescriptionTrim(command: Command, processor: QuestionProcessor) = runTest {
    val ctx = QuestionContext(
        command = command,
        state = State.NONE,
        workMode = WorkMode.TEST,
...
@OptIn(ExperimentalCoroutinesApi::class)
fun validationDescriptionEmpty(command: Command, processor: QuestionProcessor) = runTest {
    val ctx = QuestionContext(
        command = command,
        state = State.NONE,
        workMode = WorkMode.TEST,
...
@OptIn(ExperimentalCoroutinesApi::class)
fun validationDescriptionSymbols(command: Command, processor: QuestionProcessor) = runTest {
    val ctx = QuestionContext(
        command = command,
        state = State.NONE,
        workMode = WorkMode.TEST,
